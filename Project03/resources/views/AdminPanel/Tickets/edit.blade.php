...
<div class="container">
    ...
    <div class="head mx-auto mb-2">
        <a href="{{ route('event.show', $ticket->event->id) }}" class="back mt-auto"><i class="fa-solid fa-arrow-left"></i> Go Back</a>
        <h1 class="text-secondary text-end fst-italic m-0 mt-auto ">Edit Ticket</h1>
    </div>
    <hr>


    <form id="edit-ticket-form" data-id="{{ $ticket->id }}" class="form-column d-flex align-itemns-start justify-content-between flex-wrap mx-auto">
        @csrf
        @METHOD('PUT')
        ...
        <div class="coolinput">
            ...
            <select name="ticket_type" id="ticket_type" class="input">
                ...
                <option value="person" {{ $ticket->ticket_type === 'person' ? 'selected' : ''}}>Person</option>
                <option value="company" {{ $ticket->ticket_type === 'company' ? 'selected' : ''}}>Company</option>
            </select>
        ...
        <div class="coolinput">
            ...
            <input type="text" name="price" id="price" class="input" placeholder="Write price..." value="{{ $ticket->price }}">
            <small id="price-error" class="error-message text-danger fw-bold d-block"></small>
        </div>

        <div class="coolinput">
            <label for="quantity" class="text">Quantity</label>
            <input type="number" name="quantity" id="quantity" class="input" placeholder="Write quantity number..." value="{{ $ticket->quantity }}">
            <small id="quantity-error" class="error-message text-danger fw-bold d-block"></small>
        </div>

        <div class="coolinput">
            <label for="seats" class="text">Seats</label>
            <input type="number" name="seats" id="seats" class="input" placeholder="Write seats number..." value="{{$ticket->seats}}">
            <small id="seats-error" class="error-message text-danger fw-bold d-block"></small>
        </div>

        <div class="coolinput">
            <label for="pauses" class="text">Pauses</label>
            <textarea name="pauses" id="pauses" class="input" placeholder="Write pauses...">{{$ticket->pauses}}</textarea>
            <small id="pauses-error" class="error-message text-danger fw-bold d-block"></small>
        </div>

        <div class="coolinput">
            <label for="wifi" class="text">WiFi</label>
            <input type="text" name="wifi" id="wifi" class="input" placeholder="0/1" value="{{$ticket->wifi}}">
